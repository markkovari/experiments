...
pub fn build(b: *std.Build) void {
    ...
    const mod = b.addModule("zig_state_machine", .{
    ...
    // --- Domain layer: pure entities and rules, zero deps beyond std. ---
    // Declared with addModule so future layers (usecases, sqlite) can
    // addImport("domain", domain) to depend on it by name.
    const domain = b.addModule("domain", .{
        .root_source_file = b.path("modules/domain/domain.zig"),
        .target = target,
    });
    ...
    // Domain has its own test runner. Each module is tested independently.
